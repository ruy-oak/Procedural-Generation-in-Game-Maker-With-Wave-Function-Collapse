...
for (var i = 0; i < struct_names_count(cell_types); ++i) {
	...
	if ((_type[CELL_TYPES.GRADE] == "normal") || (_type[CELL_TYPES.GRADE] == "special")){
...
do {
	...
	while(_has_neighbors) == false {
		...
		if (array_length(_array_choices) == 0){
			...
			while(_new_way) == false {
				...
				if (!_right_neightborhood && !_left_neightborhood && !_up_neightborhood && !_down_neightborhood){
					i ++;	
					//if (global.dungeon_cells[# _pos_x, _pos_y] != 1) global.dungeon_cells[# _pos_x, _pos_y] = 2; // caminho sem saída
					//show_message("a")
				} else {
					_x = _pos_x;
					_y = _pos_y;
		...
		//	_has_neighbors = true; 
		//} else {
		//	if (global.dungeon_cells[# _possible_x, _possible_y] != 1) global.dungeon_cells[# _possible_x, _possible_y] = 2; // caminho sem saída
		//}
	}
...
global.dungeon_cell_types[# global.dungeon_furthest_x, global.dungeon_furthest_y] = cell_types.red_torii[CELL_TYPES.TYPE]; // Definindo o tipo da sala final
